...
export default function handleData(json: SensorData, temperature: any[][], thermalCamera: ThermalCamera, pane3d: Pane3D) {
	...
	let tempValue = Math.ceil((4 - 1) / (35 - 20) * json.temperature.temperature - 4)
	let flameValue = Math.ceil((2 - 1) / (200 - 50) * json.gas.tvoc - 1 / 3)
	...
	document.getElementById("temp").style.backgroundImage = `url(./data/icons/temp${tempValue}.png)`
	document.getElementById("flame").style.backgroundImage = `url(./data/icons/flame${flameValue}.png)`
	document.getElementById("cloud").style.backgroundImage = `url(./data/icons/cloud${cloudValue}.png)`
	document.getElementById("lightbulb").style.backgroundImage = `url(./data/icons/lightbulb${lightbulbValue}.png)`
	document.getElementById("proximity").style.backgroundImage = `url(./data/icons/proximity${proximityValue}.png)`
}